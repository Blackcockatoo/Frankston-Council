(() => {
  ...
  function mediaPreview(item) {
    ...
    return `<img class="card-media" src="${escapeHtml(item.image)}" alt="${escapeHtml(item.title)}" loading="lazy" decoding="async" />`;
  }
    ...
    return `
      <section class="old-vic-feature" aria-labelledby="old-vic-feature-title">
        <figure class="old-vic-hero">
          <img src="14_music_campaigns/old-vic-state-campaign.png" alt="Old Vic State in monumental gold lettering between a recording studio and a gothic civic procession" loading="lazy" decoding="async">
          <figcaption>
        ...
        <div class="old-vic-chapters">
          ${chapters.map(([number, title, copy, image]) => `
            <figure>
              <img src="${image}" alt="${escapeHtml(title)} artwork from the Old Vic State cycle" loading="lazy" decoding="async">
